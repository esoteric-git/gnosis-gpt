...
def chat_with_doc(model, collection, stats_db):
        ...
        if not st.session_state["overused"]:
            ...
            if model.startswith("gpt"):
                ...
                qa = ConversationalRetrievalChain.from_llm(
                    OpenAI(
                        model_name=st.session_state['model'], openai_api_key=openai_api_key, temperature=st.session_state['temperature'], max_tokens=st.session_state['max_tokens']), ChromaRetriever(collection), memory=memory, verbose=True)
            elif anthropic_api_key and model.startswith("claude"):
                logger.info('Using Anthropics model %s', model)
                qa = ConversationalRetrievalChain.from_llm(
                    ChatAnthropic(
                        model=st.session_state['model'], anthropic_api_key=anthropic_api_key, temperature=st.session_state['temperature'], max_tokens_to_sample=st.session_state['max_tokens']), ChromaRetriever(collection), memory=memory, verbose=True, max_tokens_limit=102400)
...
class ChromaRetriever:
    ...
    def get_relevant_documents(self, query):
        query_embedding = []  # Generate embedding using OpenAIEmbeddings or any other method
        results = query_collection(self.collection, query_embedding)
        documents = [{"content": result['content'], "metadata": result['metadata']} for result in results]
        return documents
